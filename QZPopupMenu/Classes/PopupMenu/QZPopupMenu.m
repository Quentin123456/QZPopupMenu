...
#import "QZPopupMenuPagenatorView.h"

static const NSTimeInterval kQZPopupMenuAnimationDuration = 0.2;

@interface QZPopupMenu ()

@property (assign, nonatomic, getter = isVisible, readwrite) BOOL visible;
@property (strong, nonatomic) QZPopupMenuOverlayView *overlayView;

@property (weak, nonatomic) UIView *view;
@property (assign, nonatomic) CGRect targetRect;

@property (assign, nonatomic) NSUInteger page;
@property (assign, nonatomic) QZPopupMenuArrowDirection actualArrorDirection;
@property (assign, nonatomic) CGPoint arrowPoint;

@property (strong, nonatomic) NSMutableArray *itemViews;
@property (strong, nonatomic) NSMutableArray *groupedItemViews;
@property (strong, nonatomic) NSMutableArray *visibleItemViews;

@end

@implementation QZPopupMenu

+ (Class)itemViewClass {
    
    return [QZPopupMenuItemView class];
    
}

+ (Class)pagenatorViewClass {
    
    return [QZPopupMenuPagenatorView class];
...
- (instancetype)initWithItems:(NSArray *)items {
    ...
    if (self) {
        ...
        self.cornerRadius = 8;
        self.arrowSize = 9;
        self.arrowDirection = QZPopupMenuArrowDirectionDefault;
...
- (void)showInView:(UIView *)view targetRect:(CGRect)targetRect animated:(BOOL)animated {
    ...
    QZPopupMenuArrowDirection arrowDirection = self.arrowDirection;
    
    if (arrowDirection == QZPopupMenuArrowDirectionDefault) {
        if ((targetRect.origin.y - (self.height + self.arrowSize)) >= self.popupMenuInsets.top) {
            arrowDirection = QZPopupMenuArrowDirectionDown;
            ...
            arrowDirection = QZPopupMenuArrowDirectionUp;
        } else {
            ...
            arrowDirection = (left > right) ? QZPopupMenuArrowDirectionLeft : QZPopupMenuArrowDirectionRight;
        }
    }
    ...
    switch (arrowDirection) {
            case QZPopupMenuArrowDirectionDown:
            case QZPopupMenuArrowDirectionUp:
            ...
            case QZPopupMenuArrowDirectionLeft:
            maximumWidth = targetRect.origin.x - self.popupMenuInsets.left;
            break;
            
            case QZPopupMenuArrowDirectionRight:
    ...
    // Create overlay view
    self.overlayView = ({
        QZPopupMenuOverlayView *overlayView = [[QZPopupMenuOverlayView alloc] initWithFrame:view.bounds];
    ...
    if (animated) {
        ...
        [UIView animateWithDuration:kQZPopupMenuAnimationDuration animations:^(void) {
            self.alpha = 1.0;
...
- (void)dismissAnimated:(BOOL)animated
{
    ...
    if (animated) {
        [UIView animateWithDuration:kQZPopupMenuAnimationDuration animations:^{
            self.alpha = 0;
        } completion:^(BOOL finished) {
...
- (void)createItemViews {
    ...
    for (QZPopupMenuItem *item in self.items) {
        QZPopupMenuItemView *itemView = [[[self class] itemViewClass] itemViewWithItem:item];
...
- (void)resetItemViewState:(QZPopupMenuItemView *)itemView {
...
- (void)groupItemViewsWithMaximumWidth:(CGFloat)maximumWidth {
    ...
    CGFloat pagenatorWidth = [QZPopupMenuPagenatorView pagenatorWidth];
    
    // Create new array
    NSMutableArray *itemViews = [NSMutableArray array];
    CGFloat width = 0;
    if (self.actualArrorDirection == QZPopupMenuArrowDirectionLeft || self.actualArrorDirection == QZPopupMenuArrowDirectionRight) {
        width += self.arrowSize;
    }
    
    for (QZPopupMenuItemView *itemView in self.itemViews) {
        // Clear state
...
- (void)updateVisibleItemViewsWithPage:(NSUInteger)page {
    ...
    if (numberOfPages > 1 && page != 0) {
        QZPopupMenuPagenatorView *leftPagenatorView = [[[self class] pagenatorViewClass] leftPagenatorViewWithTarget:self action:@selector(showPreviousPage)];
    ...
    for (QZPopupMenuItemView *itemView in itemViews) {
        [self addSubview:itemView];
        [visibleItemViews addObject:itemView];
    }
    
    if (numberOfPages > 1 && page != numberOfPages - 1) {
        QZPopupMenuPagenatorView *rightPagenatorView = [[[self class] pagenatorViewClass] rightPagenatorViewWithTarget:self action:@selector(showNextPage)];
...
- (void)layoutVisibleItemViews
{
    ...
    if (self.actualArrorDirection == QZPopupMenuArrowDirectionDown || self.actualArrorDirection == QZPopupMenuArrowDirectionUp) {
        height += self.arrowSize;
    }
    
    CGFloat offset = 0;
    for (NSInteger i = 0; i < self.visibleItemViews.count; i++) {
        QZPopupMenuItemView *itemView = [self.visibleItemViews objectAtIndex:i];
        
        // Clear state
        [self resetItemViewState:itemView];
        
        // Set item view insets
        if (i == 0 && self.actualArrorDirection == QZPopupMenuArrowDirectionLeft) {
            itemView.button.contentEdgeInsets = UIEdgeInsetsMake(0, self.arrowSize, 0, 0);
        } else if (i == self.visibleItemViews.count - 1 && self.actualArrorDirection == QZPopupMenuArrowDirectionRight) {
            
            itemView.button.contentEdgeInsets = UIEdgeInsetsMake(0, 0, 0, self.arrowSize);
            
        } else if (self.actualArrorDirection == QZPopupMenuArrowDirectionDown) {
            
            itemView.button.contentEdgeInsets = UIEdgeInsetsMake(0, 0, self.arrowSize, 0);
            
        } else if (self.actualArrorDirection == QZPopupMenuArrowDirectionUp) {
        ...
        if ((i == 0 && self.actualArrorDirection == QZPopupMenuArrowDirectionLeft) ||
            (i == self.visibleItemViews.count - 1 && self.actualArrorDirection == QZPopupMenuArrowDirectionRight)) {
            width += self.arrowSize;
        }
...
- (UIImage *)popupMenuImageWithHighlighted:(BOOL)highlighted
{
    ...
    // Draw body
    CGFloat y = (self.actualArrorDirection == QZPopupMenuArrowDirectionUp) ? self.arrowSize : 0;
    CGFloat height = self.height;
    ...
    for (NSInteger i = 0; i < self.visibleItemViews.count; i++) {
        ...
        if (i == 0) {
            if (self.visibleItemViews.count == 1) {
                ...
                CGRect bodyRect;
                CGRect tailRect;
                if (self.actualArrorDirection == QZPopupMenuArrowDirectionLeft) {
                ...
                else if (self.actualArrorDirection == QZPopupMenuArrowDirectionRight) {
            ...
            CGRect bodyRect;
            CGRect tailRect;
            if (self.actualArrorDirection == QZPopupMenuArrowDirectionRight) {
...
- (CGMutablePathRef)arrowPathInRect:(CGRect)rect direction:(QZPopupMenuArrowDirection)direction
{
    // Create arrow path
...
- (void)drawArrowInRect:(CGRect)rect direction:(QZPopupMenuArrowDirection)direction highlighted:(BOOL)highlighted {
    ...
    // Separator
    if (direction == QZPopupMenuArrowDirectionDown || direction == QZPopupMenuArrowDirectionUp) {
        
        for (QZPopupMenuItemView *itemView in self.visibleItemViews) {
